...
def main(k, wl, od, pc, data, location, wandb_project):
    ...
    model_0 = OrganSync_Network.load_from_checkpoint(params_0.name, map_location='cpu').double()
    ...
    O_VAE = OrganITE_Network_VAE.load_from_checkpoint(params_vae.name,  map_location='cpu').double()
    ...
    organite_net = OrganITE_Network.load_from_checkpoint(params_oite.name, map_location='cpu').double()
