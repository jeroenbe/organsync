...
# OWN MODULES
from .utils import UNOS_2_UKReg_mapping as mapping
from .utils import o_cols, o_cols_unos_ukeld, x_cols, x_cols_unos_ukeld


def _make_liver_data(location: str, destination: str, replace_organ: int) -> None:
    ...
    liver.loc[liver.TX_YEAR.isnull(), o_cols_intersected] = liver.loc[
        liver.TX_YEAR.isnull(), o_cols_intersected
    ].replace(np.nan, replace_organ)
    ...
    old_variables = [
        "PREV_PI_TX_TCR_ARCHIVE",
        "PGE",
        "OTH_DON_MED1_OSTXT_DON_OLD",
        "OTH_DON_MED2_OSTXT_DON_OLD",
        "OTH_DON_MED3_OSTXT_DON_OLD",
        "MRCREATG_OLD",
        "IABP",
        "HEPD_OLD",
        "HBEAB_OLD",
        ...
        "TRANSFUS_INTRAOP_NUM_OLD_DON",
        "TRANSFUS_PRIOR_NUM_OLD_DON",
        "ECMO",
    ]
    ...
    liverdrop["diag1"] = np.where(
        liverdrop.DIAG.isin([999, np.nan]), 18, liverdrop.diag1
    )
    ...
    liverdrop["DGN_TCR1"] = np.where(
        np.isnan(liverdrop.DGN_TCR), np.nan, liverdrop.DGN_TCR1
    )
    liverdrop["DGN_TCR1"] = np.where(
        liverdrop.DGN_TCR.isin([999]), 18, liverdrop.DGN_TCR1
    )
    ...
    liverdrop["DGN2_TCR2"] = np.where(
        np.isnan(liverdrop.DGN2_TCR), np.nan, liverdrop.DGN2_TCR2
    )
    liverdrop["DGN2_TCR2"] = np.where(
        liverdrop.DGN2_TCR.isin([999]), 18, liverdrop.DGN2_TCR2
    )
    ...
    liverdrop["PRI_PAYMENT_TRR1"] = np.where(
        liverdrop.PRI_PAYMENT_TRR == 1, 1, liverdrop.PRI_PAYMENT_TRR1
    )
    liverdrop["PRI_PAYMENT_TRR1"] = np.where(
        liverdrop.PRI_PAYMENT_TRR == 2, 2, liverdrop.PRI_PAYMENT_TRR1
    )
    liverdrop["PRI_PAYMENT_TRR1"] = np.where(
        np.isnan(liverdrop.PRI_PAYMENT_TRR), np.nan, liverdrop.PRI_PAYMENT_TRR1
    )

    liverdrop["PRI_PAYMENT_TCR1"] = 0
    liverdrop["PRI_PAYMENT_TCR1"] = np.where(
        liverdrop.PRI_PAYMENT_TCR == 1, 1, liverdrop.PRI_PAYMENT_TCR1
    )
    liverdrop["PRI_PAYMENT_TCR1"] = np.where(
        liverdrop.PRI_PAYMENT_TCR == 2, 2, liverdrop.PRI_PAYMENT_TCR1
    )
    liverdrop["PRI_PAYMENT_TCR1"] = np.where(
        np.isnan(liverdrop.PRI_PAYMENT_TCR), np.nan, liverdrop.PRI_PAYMENT_TCR1
    )
    ...
    liverdrop["status1"] = 0
    liverdrop["status1"] = np.where(
        liverdrop.MELD_DIFF_REASON_CD.isin([15, 16]), 1, liverdrop.status1
    )
    ...
    liverdrop["statushcc"] = np.where(
        liverdrop.MELD_DIFF_REASON_CD.isin([8]), 1, liverdrop.statushcc
    )
    liverdrop = liverdrop.drop("MELD_DIFF_REASON_CD", axis=1)

    # Coronary artery disease in donor
    liverdrop["coronary"] = 0
    liverdrop["coronary"] = np.where(
        (liverdrop.CORONARY_ANGIO_DON == "Y")
        & (liverdrop.CORONARY_ANGIO_NORM_DON == 0),
        1,
        liverdrop.coronary,
    )
    ...
    liverdrop["coronary"] = np.where(
        (liverdrop.CORONARY_ANGIO_DON == "Y")
        & (liverdrop.CORONARY_ANGIO_NORM_DON == "Y"),
        2,
        liverdrop.coronary,
    )
    ...
    liverdrop["macro"] = np.where(
        (liverdrop.LI_BIOPSY == "Y") & (liverdrop.MACRO_FAT_LI_DON < 30),
        1,
        liverdrop.macro,
    )
    ...
    liverdrop["micro"] = np.where(
        (liverdrop.LI_BIOPSY == "Y") & (liverdrop.MICRO_FAT_LI_DON < 30),
        1,
        liverdrop.micro,
    )
    ...
    liverdrop["deathcirc"] = np.where(
        liverdrop.DEATH_CIRCUM_DON == 6, 1, liverdrop.deathcirc
    )
    ...
    liverdrop["meldstat"] = np.where(
        liverdrop.END_STAT.isin([6011, 6010, 6012]), 1, liverdrop.meldstat
    )
    ...
    liverdrop["insdiab"] = 1
    liverdrop["insdiab"] = np.where(
        liverdrop.INSULIN_DEP_DON.isin([1, np.nan]), 0, liverdrop.insdiab
    )
    ...
    liverdrop["cancer"] = 1
    liverdrop["cancer"] = np.where(
        liverdrop.MALIG_TYPE.isin([np.nan]), 0, liverdrop.cancer
    )
    ...
    liverdrop["abo"] = np.where(
        liverdrop.ABO.isin(["A1B", "A2B", "AB"]), 3, liverdrop.abo
    )
    ...
    liverdrop["abodon"] = np.where(
        liverdrop.ABO_DON.isin(["A1B", "A2B", "AB"]), 3, liverdrop.abodon
    )
    ...
    # SPLIT IN SUBSETS
    train, test = train_test_split(liverdrop, test_size=0.2)
...
def _make_liver_data_u2u(
    location: str,
    destination: Optional[str] = None,
    replace_organ: Optional[int] = None,
) -> None:
    liver = pd.read_csv(location, na_values=" ")

    liver["RECEIVED_TX"] = liver.TX_YEAR.notnull()
    liver["id"] = liver.index
    liver["INIT_YEAR"] = liver.INIT_DATE.astype("datetime64[ns, US/Eastern]").dt.year

    death = 8
    liver.loc[
        (liver.RECEIVED_TX is False) & (liver.REM_CD == death), "PTIME"
    ] = (
        liver.DAYSWAIT_CHRON
    )  # patient never received organ and died -> PTIME is time on waiting list
    liver.loc[
        (liver.RECEIVED_TX is False) & (liver.REM_CD == death), "PSTATUS"
    ] = 0  # patient never received organ and died -> PSTATUS is 0 (no censor; they died)

    liver = liver[pd.notnull(liver["PTIME"])]
    liver = liver[pd.notnull(liver["PSTATUS"])]

    # ONLY USE PRESENT COLS
    x_cols_inters = np.intersect1d(liver.columns.values, x_cols_unos_ukeld)
    o_cols_inters = np.intersect1d(liver.columns.values, o_cols_unos_ukeld)
    x_cols_inters = [
        *x_cols_inters,
        "id",
        "PTIME",
    ...
    # TODO: add death_mech_don_group and deathcirc to O columns
    ...
    O.rename(columns=mapping, inplace=True)

    X.SERUM_CREATININE = np.log(X.SERUM_CREATININE)
    X.SERUM_BILIRUBIN = np.log(X.SERUM_BILIRUBIN)
    ...
    X["BILIR_SOD"] = X.SERUM_BILIRUBIN * X.SERUM_SODIUM  # X
    X["BILIR_DG"] = X.SERUM_BILIRUBIN * X.PRIMARY_LIVER_DISEASE  # X

    # liver_drop = list(liver.loc[:, liver.var() < 0.01].columns)
    # liver = liver.drop(liver_drop, 1)

    X.RCREAT = np.log(X.RCREAT)
    X.RBILIRUBIN = np.log(X.RBILIRUBIN)
    X.RINR = np.log(X.RINR)

    X = X.replace([-np.inf, np.inf], np.nan)

    X["AGE_CREAT"] = X.RAGE * X.RCREAT  # X
    X["AGE_DG"] = X.RAGE * X.PRIMARY_LIVER_DISEASE  # X

    X.rename(columns={"rwtime": "Y"}, inplace=True)

    for k in np.setdiff1d(cats_x, ["PRIMARY_LIVER_DISEASE"]):
        X.loc[:, k] = X[k].astype("category")
    for k in cats_o:
        O.loc[:, k] = O[k].astype("category")

    all_cols = np.union1d(X.columns.values, O.columns.values)
    conts = np.setdiff1d(all_cols, [*cats_x, *cats_o, "rwtime", "CENS", "RECEIVED_TX"])

    conts = [*conts, "BILIR_SOD", "BILIR_DG", "AGE_CREAT", "AGE_DG", "Y"]
    ...
    x_cols_final = np.setdiff1d(X.columns.values, ["CENS", "Y", "id"])
...
def cli(location: str, destination: str, replace_organ: int) -> None:
